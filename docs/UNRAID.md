...
Server 0.2.4 supports the published game’s **Co-op** button and public lobby list. Install this update, then open the normal game in two browsers: one player creates a co-op lobby, the other joins it. The server root also retains a connection-test page. PvP gameplay is not yet connected to the main game.

**Already running an earlier server?** Refreshing the website adds clearer teammate readiness and preserves the co-op motion improvements. Update the Docker image to **0.2.4** for shared auto rounds and the ability to change your Ready vote. Both players still start round one manually. Shared auto then counts down five real seconds between rounds; either player can switch it off, and pause/disconnect freezes the countdown. Keep the same data mount, port, environment variables, and Cloudflare route.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-install/0.2.4
cd /mnt/user/appdata/gnomeward-install/0.2.4
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.2.4/gnomeward-server-0.2.4-linux-amd64.tar.gz
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.2.4/SHA256SUMS
...
```bash
docker load -i gnomeward-server-0.2.4-linux-amd64.tar.gz
docker image inspect gnomeward-server:0.2.4 --format '{{.Os}}/{{.Architecture}}'
```

The last command should print `linux/amd64`. Keep the downloaded archive for rollback, or retain the release link. The local image is named **`gnomeward-server:0.2.4`**; it is not pulled from Docker Hub or GHCR.
...
| Repository | `gnomeward-server:0.2.4` (the image loaded above) |
| Network Type | `Bridge` |
| Privileged | Off |
...
The pinned `0.2.4` tag gives reproducible setup. Do not enable registry auto-updates for this local image; install subsequent release archives as described below. Unraid's registry update check may show an unavailable status because this image has no registry pull source.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-source/0.2.4
cd /mnt/user/appdata/gnomeward-source/0.2.4
curl -fL https://github.com/oj-kentd/gnomeward/archive/refs/tags/server-v0.2.4.tar.gz -o source.tar.gz
tar -xzf source.tar.gz --strip-components=1
docker build -f Dockerfile.server -t gnomeward-server:0.2.4 .
```
...
| Shared auto says “server update” | Load server 0.2.4, verify `/healthz` reports 0.2.4, and start a fresh room. Readiness highlighting works with older servers; shared countdowns and changing a Ready vote need the update. |
| Soul Echoes remains unavailable in co-op | Verify `/healthz` reports server 0.2.4. Find Morrow, revisit the Pumpkin Hollow cottage, and complete its later clue. A solo browser unlock does not grant a server reward. |
| Unraid tries to pull an image and fails | Load the release archive first. The Repository field must exactly match the imported local tag, `gnomeward-server:0.2.4`. This image is not on Docker Hub. |
