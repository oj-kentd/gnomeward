...
Server 0.3.4 supports the published game’s **Co-op** button and public lobby list. Install this update, then open the normal game in two browsers: one player creates a co-op lobby, the other joins it. The server root also retains a connection-test page. PvP gameplay is not yet connected to the main game.

**Already running an earlier server?** Update to **0.3.4** and refresh both browsers for Orange Knight Bramble and Skeleton Sprout costumes in co-op, alongside existing Round Coin rewards, the Skeletor necromancer costume, and the permanent boss-damage perk. Each player earns one coin per completed round; perks apply to their own gnomes. Existing combinations, enemy traits, shared countdowns, readiness, and smooth motion remain included. Keep the same data mount, port, environment variables, and Cloudflare route. The shop wallet and purchases live in each browser, so the server update does not erase them. Server 0.3.3 retains its existing shop rewards and perks but uses the original Bramble and Sprout looks. Earlier pre-shop servers still play co-op without shop rewards or perks.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-install/0.3.4
cd /mnt/user/appdata/gnomeward-install/0.3.4
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.3.4/gnomeward-server-0.3.4-linux-amd64.tar.gz
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.3.4/SHA256SUMS
...
```bash
docker load -i gnomeward-server-0.3.4-linux-amd64.tar.gz
docker image inspect gnomeward-server:0.3.4 --format '{{.Os}}/{{.Architecture}}'
```

The last command should print `linux/amd64`. Keep the downloaded archive for rollback, or retain the release link. The local image is named **`gnomeward-server:0.3.4`**; it is not pulled from Docker Hub or GHCR.
...
| Repository | `gnomeward-server:0.3.4` (the image loaded above) |
| Network Type | `Bridge` |
| Privileged | Off |
...
The pinned `0.3.4` tag gives reproducible setup. Do not enable registry auto-updates for this local image; install subsequent release archives as described below. Unraid's registry update check may show an unavailable status because this image has no registry pull source.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-source/0.3.4
cd /mnt/user/appdata/gnomeward-source/0.3.4
curl -fL https://github.com/oj-kentd/gnomeward/archive/refs/tags/server-v0.3.4.tar.gz -o source.tar.gz
tar -xzf source.tar.gz --strip-components=1
docker build -f Dockerfile.server -t gnomeward-server:0.3.4 .
```
...
| Hidden combinations behave differently in co-op | Load server 0.3.4, verify `/healthz` reports 0.3.4, refresh both browsers, and start a fresh room. |
| Shared auto says “server update” | Load server 0.3.4, verify `/healthz` reports 0.3.4, and start a fresh room. Readiness highlighting works with older servers; shared countdowns and changing a Ready vote need the update. |
| Soul Echoes remains unavailable in co-op | Verify `/healthz` reports server 0.3.4. Find Morrow, revisit the Pumpkin Hollow cottage, and complete its later clue. A solo browser unlock does not grant a server reward. |
| Unraid tries to pull an image and fails | Load the release archive first. The Repository field must exactly match the imported local tag, `gnomeward-server:0.3.4`. This image is not on Docker Hub. |
