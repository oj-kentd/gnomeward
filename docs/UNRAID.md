...
Server 0.5.0 supports the published game’s **Co-op** button and public lobby list. Install this update, then open the normal game in two browsers: one player creates a co-op lobby, the other joins it. The server root also retains a connection-test page. PvP gameplay is not yet connected to the main game.

**Already running an earlier server?** Update to **0.5.0** and refresh both browsers for expanded merge partners across every gnome and placing a shop gnome directly onto a compatible defender in co-op, including Turbo Tumble speed on merged forms, alongside existing costumes and Round Coin rewards, the Skeletor necromancer costume, and the permanent boss-damage perk. Each player earns one coin per completed round; perks apply to their own gnomes. Existing combinations, enemy traits, shared countdowns, readiness, and smooth motion remain included. Keep the same data mount, port, environment variables, and Cloudflare route. The shop wallet and purchases live in each browser, so the server update does not erase them. Server 0.4.0 supports panel and drag merging of the original four forms, but cannot buy a gnome directly onto another. Server 0.3.5 retains Turbo Tumble but cannot merge gnomes. Server 0.3.4 retains costumes and shop rewards but uses Tumble’s ordinary attack speed. Server 0.3.3 retains its existing shop rewards and perks but uses the original Bramble and Sprout looks. Earlier pre-shop servers still play co-op without shop rewards or perks.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-install/0.5.0
cd /mnt/user/appdata/gnomeward-install/0.5.0
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.5.0/gnomeward-server-0.5.0-linux-amd64.tar.gz
curl -fLO https://github.com/oj-kentd/gnomeward/releases/download/server-v0.5.0/SHA256SUMS
...
```bash
docker load -i gnomeward-server-0.5.0-linux-amd64.tar.gz
docker image inspect gnomeward-server:0.5.0 --format '{{.Os}}/{{.Architecture}}'
```

The last command should print `linux/amd64`. Keep the downloaded archive for rollback, or retain the release link. The local image is named **`gnomeward-server:0.5.0`**; it is not pulled from Docker Hub or GHCR.
...
| Repository | `gnomeward-server:0.5.0` (the image loaded above) |
| Network Type | `Bridge` |
| Privileged | Off |
...
The pinned `0.5.0` tag gives reproducible setup. Do not enable registry auto-updates for this local image; install subsequent release archives as described below. Unraid's registry update check may show an unavailable status because this image has no registry pull source.
...
```bash
mkdir -p /mnt/user/appdata/gnomeward-source/0.5.0
cd /mnt/user/appdata/gnomeward-source/0.5.0
curl -fL https://github.com/oj-kentd/gnomeward/archive/refs/tags/server-v0.5.0.tar.gz -o source.tar.gz
tar -xzf source.tar.gz --strip-components=1
docker build -f Dockerfile.server -t gnomeward-server:0.5.0 .
```
...
| Merge button says “server update” | Load server 0.5.0, verify `/healthz` reports `fusionVersion: 1`, `placementFusionVersion: 1`, and `fusionCatalogVersion: 2`, refresh both browsers, and create a new room. |
| Hidden combinations behave differently in co-op | Load server 0.5.0, verify `/healthz` reports 0.5.0, refresh both browsers, and start a fresh room. |
| Shared auto says “server update” | Load server 0.5.0, verify `/healthz` reports 0.5.0, and start a fresh room. Readiness highlighting works with older servers; shared countdowns and changing a Ready vote need the update. |
| Soul Echoes remains unavailable in co-op | Verify `/healthz` reports server 0.5.0. Find Morrow, revisit the Pumpkin Hollow cottage, and complete its later clue. A solo browser unlock does not grant a server reward. |
| Unraid tries to pull an image and fails | Load the release archive first. The Repository field must exactly match the imported local tag, `gnomeward-server:0.5.0`. This image is not on Docker Hub. |
...
After updating the container, open the usual Gnomeward playtest and select **Co-op**. Enter a nickname, select a map, and choose **Create garden**. The second player opens Co-op and clicks **Join** beside the host. Place your gnomes and have both players press **Ready**. When your teammate is ready, the Ready button glows and the small team indicators show whose turn it is. A Ready check can be clicked again to return to building. Shared auto is off initially; either player may enable it for five-second breaks after completed rounds. Both Ready votes skip the countdown. Turning auto off cancels the countdown; pausing or losing a connection freezes it. The countdown does not speed up at 2× or 3×, and victory still waits for both players to choose endless mode. Each player starts with 325 gold. Only owners can upgrade, sell, or retarget their units; lives are shared.
