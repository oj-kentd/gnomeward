...
Buying a costume does not reveal or unlock the hidden necromancer. These purchases do not change normal enemy damage or the two-path limit. Purchases cannot be bought twice or stacked.
...
The existing `gnomeward-profile` browser save gains `roundCoins`, `cosmetics`, `equippedNecroSkin`, `equippedBoomSkin`, `equippedSproutSkin`, `bossDamageUnlocked`, and `coopRoundReceipts`. Pre-shop saves migrate to an empty shop wallet while retaining their existing unlocks and records. Existing shop saves retain their coins, purchases, selected Morrow costume, and permanent perk; new costumes start unowned. Malformed balances and unknown cosmetic IDs are filtered. Data stays in the same browser; there are no accounts or cloud save synchronization. Clearing site storage removes the collection.
