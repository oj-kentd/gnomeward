...
## Permanent shop loadouts and Round Coins

Release **0.3.3** advertises `shopVersion: 1` without changing protocol 1. Create/join accepts an optional `loadout: {bossDamage: boolean, necroSkin: 'skeletor' | null}`. Unknown fields or other value types are rejected. Omission means no perk and the original costume. This casual, account-free mode trusts these two bounded browser-owned entitlements; they are not authenticated purchases. No balances, saves, character unlocks, gold, points, damage amounts, or health can be imported. Loadouts are fixed on joining and retained on reconnection; no gameplay command changes them.

The server applies boss damage once (2× maximum) only to the owning player's attacks, poison and guardians, including lingering attacks after a tower is sold. It assigns the owner's costume to necromancer towers; the cosmetic does not grant the character unlock. Each player snapshot includes `loadout`, cumulative `roundCoinsEarned`, and a unique `receiptKey` for that run and seat. Successful round completions award one coin to each player. Failed rounds award none. Clients retain the greatest paid count per receipt, so duplicate/out-of-order snapshots and reconnects cannot repeat recent rewards. Wallets, purchases and the last 128 run receipts live in browser localStorage, separate from server match records; they do not sync between devices.

Release **0.3.4** adds `costumeVersion: 1` to snapshots and `/healthz`. The optional loadout fields `boomSkin: 'orange-knight' | null` and `sproutSkin: 'skeleton' | null` select two additional owner-specific cosmetics. They do not change combat stats, unlock characters, or alter rewards. Existing clients may omit these fields. New clients check `/healthz` before a fresh connection carrying these costumes and omit only the new fields if capability is missing or the check fails; the original boss perk and necromancer costume remain intact. Reconnection retains the original room loadout without checking capabilities again.

Release **0.3.5** adds `tumbleSpeedVersion: 1` to `/healthz` and snapshots. Optional loadout `tumbleSpeed` must be boolean; true triples only the owner's Tumble attack rate, including upgraded tiers. Board `tumbleSpeedOwners` serializes the eligible owner IDs so selected-unit stats and upgrade previews agree with server combat. Clients reset this array to empty when the capability is absent. A single health request negotiates costume and Tumble support independently; unsupported fields are omitted while all supported purchases are preserved. Reconnect keeps the frozen room loadout. This power does not unlock the character or alter per-hit damage, volley size, or combination cooldowns.
