...
export class GardenRenderer {
  ...
  setGhost(type,x,z,valid,range,skin=null) {
    ...
    const modelName = type === 'necro' && skin === 'skeletor' ? 'gnome-necro-skeletor' : 'gnome-'+type;
    if(this.ghostType!==modelName){if(this.ghost){this.scene.remove(this.ghost);this.ghost.traverse(o=>{if(o.isMesh)for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();});}this.ghost=this.clone(modelName);this.ghost.traverse(o=>{if(o.isMesh){o.material=Array.isArray(o.material)?o.material.map(m=>m.clone()):o.material.clone();for(const m of Array.isArray(o.material)?o.material:[o.material]){m.transparent=true;m.opacity=.6;}}});this.scene.add(this.ghost);this.ghostType=modelName;}
