import { BOOK_ENTRIES, bookEntryDiscovered } from './merging-book.js';
...
const costumePortrait = (type, skin) => COSTUMES.find(costume => costume.towerType === type && costume.skin === skin)?.portrait || type;
const icons = {
...
export class UI {
  constructor(actions) {
    ...
    this.dialog.addEventListener('close', () => {
      // The close event is queued; a player may already have reopened it.
      if (!this.dialog.open) this.modalType = null;
    });
    document.getElementById('dialog-content').addEventListener('click', (event) => {
      if (event.target.closest('button:disabled')) return;
      if (event.target.closest('[data-merging-book]')) this.showMergingBook();
      const bookTab = event.target.closest('[data-book-tab]');
      if (bookTab) { this.bookTab = bookTab.dataset.bookTab; this.showMergingBook(); this.dialog.querySelector(`[data-book-tab="${this.bookTab}"]`)?.focus({ preventScroll: true }); }
  ...
  update(game, state = {}) {
    ...
    if (this.modalType === 'merging-book') {
      const signature = JSON.stringify([collection.fusionDiscoveries, collection.comboDiscoveries]);
      if (signature !== this.bookSignature) { const scrollTop = this.dialog.scrollTop; this.showMergingBook(); this.dialog.scrollTop = scrollTop; }
    }
    if (this.modalType === 'help') {
      ...
      if (signature !== this.traitGuideSignature) {
        ...
        if (guide) {
          ...
          content.innerHTML = enemyTraitGuide(game);
          // Keep the scrolled section attached when a discovery updates it.
          guide.replaceChildren(...content.content.firstElementChild.childNodes);
        }
    ...
    if (state.placingType) set('placement-text', state.placementMergeName ? `Merge into ${state.placementMergeName} · ${n(TOWERS[state.placingType]?.cost)} gold` : `Place ${TOWERS[state.placingType]?.name || 'your gnome'} · ${n(TOWERS[state.placingType]?.cost)} gold · Tap a compatible gnome to merge`);
    const rosterSkins = Object.fromEntries(COSTUMES.map(costume => [costume.towerType, multiplayer ? multiplayer.loadout?.[costume.towerType+'Skin'] : getTowerSkin(collection, costume.towerType)]));
    const rosterSignature = JSON.stringify([state.placingType, rosterSkins, Object.keys(TOWERS).map((id) => [game.isUnlocked(id), game.gold >= TOWERS[id].cost])]);
    if (rosterSignature !== this.rosterSignature) {
      ...
      document.getElementById('roster').innerHTML = Object.entries(TOWERS).map(([id, tower]) => {
        ...
        return `<button class="tower-card ${!unlocked ? 'locked' : !affordable ? 'unaffordable' : 'affordable'} ${secret ? 'secret-locked' : ''} ${state.placingType === id ? 'chosen' : ''}" data-tower="${id}" ${!unlocked || !affordable ? 'disabled' : ''} aria-pressed="${state.placingType === id}" title="${esc(hint)}" ${secret ? `aria-label="Map secret. ${esc(hint)}"` : ''}><div class="tower-art" style="--tower-color:${tower.color}">${portrait(costumePortrait(id, rosterSkins[id]))}${!unlocked ? `<span class="lock-badge">${icons.lock}</span>` : ''}</div><span class="tower-info"><strong>${secret ? '???' : esc(tower.name)}</strong><span class="tower-cost">${unlocked ? `${icons.coin}${n(tower.cost)}` : secret ? 'Map secret' : unlockGarden ? `Clear ${esc(unlockGarden)}` : `Round ${tower.unlockWave}`}</span></span>${!affordable && unlocked ? '<span class="card-shortage">Need gold</span>' : ''}</button>`;
      }).join('');
    }
  ...
  renderSelection(game, tower) {
    ...
    const recipes = Object.values(FUSIONS).filter(recipe=>!recipe.secret&&recipe.types.length>members.length&&members.every(member=>recipe.types.includes(member.type)));
    const mergeSupported = !multiplayer || (multiplayer.fusionsSupported&&recipes.some(recipe=>(recipe.catalogVersion||1)<=multiplayer.fusionCatalogVersion));
    const candidates = state.mergeOpen && members.length < 3 && mergeSupported && owned ? game.towers.filter(other => {
      const candidate=FUSIONS[[...members,...game.getFusionMembers(other.id)].map(t=>t.type).sort().join('-')];
      return !other.fusionParentId&&candidate&&!candidate.secret&&(!multiplayer||(candidate.catalogVersion||1)<=multiplayer.fusionCatalogVersion)&&game.canMergeTowers(root.id,other.id);
    }) : [];
    ...
    const mergeControls = recipes.length > 0 && owned ? `<section class="fusion-controls"><button class="fusion-open" data-merge-open aria-expanded="${!!state.mergeOpen}" ${!canEdit || !mergeSupported ? 'disabled' : ''}>${state.mergeOpen ? 'Close merging' : 'Merge gnomes ✦'}</button>${!mergeSupported ? '<p>Update the co-op server to merge gnomes.</p>' : state.mergeOpen ? `<p>Merge compatible gnomes at any upgrade level. ${!multiplayer || multiplayer.placementFusionsSupported ? 'Drag one onto another, or place a shop gnome onto a compatible defender.' : 'Drag one placed gnome onto another, or choose a partner below. Update the server for direct shop placement.'} Each keeps its abilities and upgrades.</p>${candidates.length ? `<div class="fusion-candidates">${candidates.map(other => `<button data-merge-partner="${other.id}" aria-pressed="${other.id === partner?.id}">${portrait(FUSIONS[other.fusionKey]?.portrait || other.type)}<span>${esc(FUSIONS[other.fusionKey]?.name || TOWERS[other.type].name)}<small>Gnome #${other.id}</small></span></button>`).join('')}</div>` : '<p class="fusion-empty">Choose a compatible gnome from the shop and place it onto this one, or plant it elsewhere to select it here. Only your own gnomes can merge, with one of each kind.</p>'}${recipe ? `<div class="fusion-preview">${portrait(recipe.portrait)}<strong>${esc(recipe.name)}</strong><p>Both defenders become one at this selected gnome’s spot. Keeps every ability and upgrade. Costumes are put aside; your purchases stay yours.</p><p>Free · permanent for this game</p><button class="primary-button" data-merge-confirm ${!canEdit ? 'disabled' : ''}>MERGE INTO ${esc(recipe.name.toUpperCase())}</button></div>` : ''}` : ''}</section>` : '';
  ...
  showMergingBook() {
    const collection = this.last?.state.shopProfile || this.last?.game.profile;
    if (!collection?.mergingBookFound) return;
    this.bookTab = this.bookTab === 'combos' ? 'combos' : 'fusions';
    this.bookSignature = JSON.stringify([collection.fusionDiscoveries, collection.comboDiscoveries]);
    const known = BOOK_ENTRIES.filter(entry => bookEntryDiscovered(collection, entry)).length;
    const entryMarkup = (entry, index) => {
      const discovered = bookEntryDiscovered(collection, entry);
      const art = discovered && entry.portrait ? portrait(entry.portrait) : entry.types.map(type => portrait(type)).join('');
      return `<article class="book-entry ${discovered ? 'book-discovered' : 'book-unknown'}" data-book-entry="${index}"><div class="book-entry-art" ${!discovered ? 'aria-label="Unknown gnome silhouettes"' : ''}>${art}${!discovered ? '<b aria-hidden="true">?</b>' : ''}</div><div class="book-entry-copy"><h3>${discovered ? esc(entry.name) : 'An unwritten discovery'}</h3>${discovered ? `<p class="book-recipe">${esc(entry.recipe)}</p><p>${esc(entry.description)}</p>` : `<p class="book-hint">${esc(entry.hint)}</p><small>Experiment in the garden to reveal this page.</small>`}</div></article>`;
    };
    this.openModal('merging-book', `<section class="merging-book" id="merging-book"><div class="modal-heading"><div><span class="eyebrow">A GARDENER’S LOST NOTES</span><h2>Book of Merging</h2></div><button class="modal-close" data-close aria-label="Close Book of Merging">×</button></div><p class="book-intro">Some powers become one. Others bloom side by side. Let your discoveries fill these pages.</p><p class="book-progress" role="status">${known} / ${BOOK_ENTRIES.length} discoveries recorded</p><div class="book-tabs" role="tablist" aria-label="Book chapters"><button role="tab" data-book-tab="fusions" aria-selected="${this.bookTab === 'fusions'}" aria-controls="book-fusions">Merged forms</button><button role="tab" data-book-tab="combos" aria-selected="${this.bookTab === 'combos'}" aria-controls="book-combos">Secret combos</button></div>${['fusions', 'combos'].map(tab => `<div id="book-${tab}" class="book-pages" role="tabpanel" ${this.bookTab !== tab ? 'hidden' : ''}>${BOOK_ENTRIES.map((entry, index) => ((entry.kind === 'fusion') === (tab === 'fusions')) ? entryMarkup(entry, index) : '').join('')}</div>`).join('')}<p class="book-footnote">Your book stays in this browser, across gardens and co-op games.</p><button class="primary-button" data-close>BACK TO THE GARDEN</button></section>`);
  }
    ...
    this.openModal('coin-shop', `<section id="round-coin-shop" class="coin-shop">
      ...
      <div class="coin-shop-grid">
        ...
        ${COSTUMES.map(costume => {
          ...
          const equipped = getTowerSkin(profile, costume.towerType) === costume.skin;
          const name = TOWERS[costume.towerType].name;
          return `<article class="coin-shop-card skin-card" data-costume-card="${item.id}"><div class="shop-product-art costume-${costume.towerType}">${portrait(costume.portrait)}<span class="shop-product-badge">${esc(name.toUpperCase())} COSTUME</span></div>
            <div class="shop-product-copy"><h3>${esc(item.name)}</h3><p>${esc(item.description)}</p><small>Cosmetic only · ${game.isUnlocked(costume.towerType) ? `${esc(name)} is ready to dress up.` : 'Find the necromancer in a garden to use this costume.'}</small></div>
            <div class="shop-product-action">${owned ? `<span class="shop-owned">✓ Yours forever</span><div class="costume-options" role="group" aria-label="${esc(name)} costume"><button data-shop-tower="${costume.towerType}" data-shop-skin="default" aria-pressed="${!equipped}" ${multiplayer ? 'disabled' : ''}>Original</button><button data-shop-tower="${costume.towerType}" data-shop-skin="${costume.skin}" aria-pressed="${equipped}" ${multiplayer ? 'disabled' : ''}>${equipped ? '✓ Equipped' : 'Equip costume'}</button></div>` : purchase(item)}</div>
          </article>`;
        }).join('')}
